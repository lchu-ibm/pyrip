...
def optimize(infile, outfile, block_size=512, compression='LZW', predictor=2, big_tiff=True,
    ...
    if infile.startswith('/vsis3'):
        if access_key_id is None or secret_access_key is None or endpoint is None:
            raise ValueError('access_key_id, secret_access_key and endpoint must be set when infile is from cloud storage')
...
def build_vrt(outfile, infiles, access_key_id=None, secret_access_key=None, endpoint=None):
    ...
    if any(f.startswith('/vsis3') for f in infiles) or outfile.startswith('/vsis3'):
        if access_key_id is None or secret_access_key is None or endpoint is None:
            raise ValueError('access_key_id, secret_access_key and endpoint must be set when any infile or outfile is from cloud storage')
        args.extend(['--config', 'AWS_ACCESS_KEY_ID', access_key_id])
        args.extend(['--config', 'AWS_SECRET_ACCESS_KEY', secret_access_key])
        args.extend(['--config', 'AWS_S3_ENDPOINT', endpoint.replace('https://', '')])
...
def check_info(file, access_key_id=None, secret_access_key=None, endpoint=None):
    ...
    if file.startswith('/vsis3'):
        if access_key_id is None or secret_access_key is None or endpoint is None:
            raise ValueError('access_key_id, secret_access_key and endpoint must be set when file is from cloud storage')
        args.extend(['--config', 'AWS_ACCESS_KEY_ID', access_key_id])
        args.extend(['--config', 'AWS_SECRET_ACCESS_KEY', secret_access_key])
        args.extend(['--config', 'AWS_S3_ENDPOINT', endpoint.replace('https://', '')])
    try:
